...
part of 'view_test.dart';

// **************************************************************************
// FloorGenerator
// **************************************************************************

abstract class $ViewTestDatabaseBuilderContract {
  /// Adds migrations to the builder.
  $ViewTestDatabaseBuilderContract addMigrations(List<Migration> migrations);

  /// Adds a database [Callback] to the builder.
  $ViewTestDatabaseBuilderContract addCallback(Callback callback);

  /// Creates the database and initializes it.
  Future<ViewTestDatabase> build();
}

// ignore: avoid_classes_with_only_static_members
class $FloorViewTestDatabase {
  /// Creates a database builder for a persistent database.
  /// Once a database is built, you should keep a reference to it and re-use it.
  static $ViewTestDatabaseBuilderContract databaseBuilder(String name) =>
      _$ViewTestDatabaseBuilder(name);

  /// Creates a database builder for an in memory database.
  /// Information stored in an in memory database disappears when the process is killed.
  /// Once a database is built, you should keep a reference to it and re-use it.
  static $ViewTestDatabaseBuilderContract inMemoryDatabaseBuilder() =>
      _$ViewTestDatabaseBuilder(null);
}

class _$ViewTestDatabaseBuilder implements $ViewTestDatabaseBuilderContract {
  _$ViewTestDatabaseBuilder(this.name);
  ...
  @override
  Future<ViewTestDatabase> build() async {
    final path = name != null
        ? await sqfliteDatabaseFactory.getDatabasePath(name!)
        : ':memory:';
    final database = _$ViewTestDatabase();
...
class _$ViewTestDatabase extends ViewTestDatabase {
  _$ViewTestDatabase([StreamController<String>? listener]) {
  ...
  Future<sqflite.Database> open(
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        ...
        await database.execute(
            'CREATE VIEW IF NOT EXISTS `names` AS SELECT custom_name as name FROM person UNION SELECT name from dog');
        await database.execute(
            'CREATE VIEW IF NOT EXISTS `multiline_query_names` AS     SELECT custom_name as name \n    FROM person \n    UNION SELECT name from dog\n  ');
  ...
  @override
  NameDao get nameDao {
    return _nameDaoInstance ??= _$NameDao(database, changeListener);
  }
}
  ...
  Future<void> replacePersons(List<Person> persons) async {
    if (database is sqflite.Transaction) {
      ...
      await (database as sqflite.Database)
        ...
        final transactionDatabase = _$ViewTestDatabase(changeListener)
...
class _$NameDao extends NameDao {
  _$NameDao(
    this.database,
    this.changeListener,
  ) : _queryAdapter = QueryAdapter(database, changeListener);
  ...
  Future<List<Name>> findAllNames() async {
    return _queryAdapter.queryList('SELECT * FROM names ORDER BY name ASC',
  ...
  @override
  Future<Name?> findExactName(String name) async {
    return _queryAdapter.query('SELECT * FROM names WHERE name = ?1',
        mapper: (Map<String, Object?> row) => Name(row['name'] as String),
        arguments: [name]);
  }

  @override
  Future<List<Name>> findNamesLike(String suffix) async {
    return _queryAdapter.queryList(
        'SELECT * FROM names WHERE name LIKE ?1 ORDER BY name ASC',
        mapper: (Map<String, Object?> row) => Name(row['name'] as String),
        arguments: [suffix]);
  }

  @override
  Future<List<Name>> findNamesMatchingBoth(
    String prefix,
    String suffix,
  ) async {
    return _queryAdapter.queryList(
        'SELECT * FROM names WHERE name LIKE ?2 AND name LIKE ?1 ORDER BY name ASC',
        mapper: (Map<String, Object?> row) => Name(row['name'] as String),
        arguments: [prefix, suffix]);
  }

  @override
  Future<MultilineQueryName?> findMultilineQueryName(String name) async {
    return _queryAdapter.query(
        'SELECT * FROM multiline_query_names WHERE name = ?1',
        mapper: (Map<String, Object?> row) =>
            MultilineQueryName(row['name'] as String),
        arguments: [name]);
  }
}
