...
class VariableVisitor extends RecursiveVisitor<void,void>{
  ...
  VariableVisitor(this._queryMethod,{this.numberedVarsAllowed=false, this.checkIfVariableExists});
  ...
  void visitNumberedVariable(NumberedVariable e, void arg) {
    ...
    if (!numberedVarsAllowed){
      throw InvalidGenerationSourceError(
          ...
          todo: 'Please use a named variable (`:name`) instead of numbered variables (`?` or `?3`).',
  ...
  void visitNamedVariable(ColonNamedVariable e, void arg) {
    if(checkIfVariableExists!=null && !checkIfVariableExists.contains(e.name)){
      throw InvalidGenerationSourceError(
          'Invalid named variable $e in statement of `@Query` annotation does not exist in the method parameters.',
          todo: 'Please add a method parameter for the variable $e with the name ${e.name.substring(1)}.',
          element: _queryMethod
      );
    }
    variables.add(e);
