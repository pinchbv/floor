...
/// them.
class ReferencedQueryablesVisitor extends RecursiveVisitor<void, void> {
  /// All tables that have been referenced anywhere in this query.
  final Set<NamedResultSet> foundTables = {};
  ...
  void visitReference(Reference e, void arg) {
    ...
    if (column is ViewColumn) {
      foundTables.add(column.view);
    }

    visitChildren(e, arg);
  }

  NamedResultSet /*?*/ _toNRSOrNull(ResolvesToResultSet resultSet) {
  ...
  void visitQueryable(Queryable e, void arg) {
    if (e is TableReference) {
      final resolved = _toNRSOrNull(e.resultSet);
      if (resolved != null) {
...
class UpdatedTablesVisitor extends ReferencedQueryablesVisitor {
...
Set<NamedResultSet> findReferencedTablesOrViews(AstNode root) {
  return (ReferencedQueryablesVisitor()..visit(root, null)).foundTables;
}