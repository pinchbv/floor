...
class AnalyzerEngine{
  ...
  void registerEntity(Entity entity){
    ...
    registry.addAll({entity.name : entity as Queryable});
  ...
  void registerView(floor.View floorView){
    registry.addAll({floorView.name : floorView as Queryable});
  ...
  static AnalyzeStatementOptions _methodToNamedVariables(@nonNull MethodElement method){
    return AnalyzeStatementOptions(namedVariableTypes: Map.fromEntries(method.parameters.map((param) => MapEntry(':${param.name}',_getSqlparserType(param, flattenLists: true)))));
  }
  ...
  @nonNull
  static ResolvedType _getSqlparserType(VariableElement parameter, {bool flattenLists = false}) {
    ...
    if (type.isDartCoreInt) {
      ...
      return const ResolvedType(type: BasicType.int, nullable: true, hint: IsBoolean());
  ...
  AnalyzeResult _generateNewStatement(String query, AnalysisContext ctx, MethodElement method){
    ...
    final visitor = VariableVisitor(method, checkIfVariableExists:indices.keys, numberedVarsAllowed: false)
